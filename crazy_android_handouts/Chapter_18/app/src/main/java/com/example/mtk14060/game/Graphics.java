package com.example.mtk14060.game;
...
//定义画图的工具类（该类是对Android绘图的Canvas的一个封装）
public class Graphics
{
    ...
    //定义绘制矩形的方法，暂未用到
    public static void drawRect(Canvas canvas, float x, float y, float w, float h, Paint paint)
    {
        paint.setStyle(Paint.Style.STROKE);
    ...
    //定义绘制弧线的方法，暂未用到
    public static void drawArc(Canvas canvas, float x, float y, float width, float height,
    ...
    //定义绘制字符串的方法
    public static void drawString(Canvas canvas, String text, float textSize, float x, float y, int anchor, Paint paint)
    {
        //动态计算字符串的对齐方式
        if ((anchor & LEFT) != 0)
        {
    ...
    /**
     * 绘制包边字符串的方法,先采用描边方式绘制字符串，再采用填充方式绘制字符串，就可以形成包边字符串
     *
     * @param canvas           绘制包边字符串的画布
     ...
     */
    public static void drawBorderString(Canvas canvas, int borderColor, int textColor, String text,
                                        int x, int y, int borderWidth, Paint paint)
    {
    ...
    // 工具方法：绘制位图
    // 作用是将源位图image中左上角为srcX、srcY、宽width、高height的区域绘制到canvas的destX/destY处
